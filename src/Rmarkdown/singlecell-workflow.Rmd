...
kelly_col <- unname(pals::kelly()[-c(1,2,3,6)])
...
plotReducedDim(sce, dimred="TSNE", other_fields = "Sequencing", colour_by = "labels_pruned") + facet_wrap(~Sequencing) + scale_colour_manual(values = kelly_col)
```
...
```{r}
chisq.test(table(sce$Sequencing, sce$labels_pruned))
```

```{r}
prop <- as.data.frame(table(sce$Sequencing, sce$labels_pruned))
